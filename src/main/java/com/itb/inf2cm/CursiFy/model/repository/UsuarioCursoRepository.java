...
public interface UsuarioCursoRepository extends JpaRepository<UsuarioCurso, Long> {
    ...
    @Query(value = "SELECT TOP 1 uc1.curso_id, c.nome FROM UsuarioCurso uc1 JOIN UsuarioCurso uc2 ON uc1.curso_id = uc2.curso_id JOIN Curso c ON c.id = uc1.curso_id JOIN Usuario u1 ON u1.id = uc1.usuario_id JOIN Usuario u2 ON u2.id = uc2.usuario_id WHERE uc1.usuario_id = :userA AND uc2.usuario_id = :userB AND ((UPPER(u1.nivel_acesso) IN ('PROFESSOR','TEACHER') AND UPPER(u2.nivel_acesso) IN ('ALUNO','STUDENT')) OR (UPPER(u1.nivel_acesso) IN ('ALUNO','STUDENT') AND UPPER(u2.nivel_acesso) IN ('PROFESSOR','TEACHER')))", nativeQuery = true)
    Object[] findSharedCourse(@Param("userA") Long userA, @Param("userB") Long userB);

    @Query(value = "SELECT u.id, u.nome, u.email, u.nivel_acesso, STRING_AGG(c.nome, ', ') AS cursos FROM Usuario u JOIN UsuarioCurso other ON other.usuario_id = u.id JOIN UsuarioCurso mine ON mine.curso_id = other.curso_id JOIN Curso c ON c.id = mine.curso_id JOIN Usuario me ON me.id = mine.usuario_id WHERE me.id = :userId AND u.id <> :userId AND ((UPPER(me.nivel_acesso) IN ('ALUNO','STUDENT') AND UPPER(u.nivel_acesso) IN ('PROFESSOR','TEACHER')) OR (UPPER(me.nivel_acesso) IN ('PROFESSOR','TEACHER') AND UPPER(u.nivel_acesso) IN ('ALUNO','STUDENT'))) GROUP BY u.id, u.nome, u.email, u.nivel_acesso", nativeQuery = true)
